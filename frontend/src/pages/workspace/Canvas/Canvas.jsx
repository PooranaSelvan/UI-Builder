...
const Canvas = ({ components, zoom, selectedComponentId, onSelectComponent, clearComponentSelection }) => {
  ...
  return (
    <div className="work-canvas-wrapper" onClick={unSelectComponent}>
      <div ref={setNodeRef} className="canvas-drop-zone" style={{ flex: 1, transform: `scale(${zoom})`, transformOrigin: "top left", transition: "transform 0.3s ease-in-out", padding: "20px", background: isOver ? "#eaf7ff" : "white", border: isOver ? "2px dashed black" : "" }}>
        {components.length === 0 && (
