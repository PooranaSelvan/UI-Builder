...
const Workspace = () => {
  ...
  const handleDragEnd = (event) => {
    ...
    // Adding Child Component from Left SideBar
    if (isFromSidebar && over.id !== "canvas") {
      let componentData = active.data.current.component;
    ...
    if (!isFromSidebar && over.id !== "canvas") {
      ...
      setComponents(items => {
        ...
        let itemToMove = componentData;

        if (isChild) {
          let result = removeChild(items, active.id);
          updatedComponents = result.newComponent;
          itemToMove = result.child;
        } else {
          updatedComponents = items.filter(item => item.id !== active.id);
        }

        return addChildToComponent(updatedComponents, over.id, itemToMove);
      });
  ...
  const updateComponent = (id, updates) => {
    setComponents(prevComponents => {
      const copyComponents = cloneComponents(prevComponents);
      let arr = [...copyComponents];
  ...
  const cloneComponents = (items) => {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      if (item.children && item.children.length > 0) {
        itemObj.children = cloneComponents(item.children);
      }
